$(document).ready(function () {
    // Function to get clean export title
    function getExportTitle() {
        var label = $('.download_label').text().trim();
        return label ? label : 'Report_Export';
    }

    // Initialize standard DataTables on elements with .example class
    if ($('.example').length > 0) {
...
// dataSrc="data" it is array source of data
   ...
   function emptyDatatable(_selector,dataSrc="data"){
        ...
        "info":     true,
        "ajax": {
            "url": baseurl+'assets/json-files/datatable_empty.json',
            "dataSrc": dataSrc
        }
    });
    }